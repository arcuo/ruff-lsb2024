...
use crate::rules::{
    ...
    flake8_tidy_imports, flake8_trio, flake8_type_checking, information_flow, mccabe, pandas_vet,
    pep8_naming, perflint, pycodestyle, pyflakes, pygrep_hooks, pylint, pyupgrade, refurb, ruff,
    tryceratops,
};
...
pub(crate) fn statement(stmt: &Stmt, checker: &mut Checker) {
    match stmt {
        ...
        Stmt::Assign(
            ...
            if checker.enabled(Rule::IFExplicitVariableAssign) {
                information_flow::rules::check_if_assign_targets_statement(
                    checker,
                    &assign.targets,
                    &assign.value,
                    &checker.settings.information_flow.security_property,
